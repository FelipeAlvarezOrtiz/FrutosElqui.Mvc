// <auto-generated />
using System;
using FrutosElqui.Persistencia;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Metadata;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;

namespace FrutosElqui.Persistencia.Migrations
{
    [DbContext(typeof(ApplicationDbContext))]
    [Migration("20211112143722_FixOfertaDetalle")]
    partial class FixOfertaDetalle
    {
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("Relational:MaxIdentifierLength", 128)
                .HasAnnotation("ProductVersion", "5.0.11")
                .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

            modelBuilder.Entity("FrutosElqui.Core.Clientes.Cliente", b =>
                {
                    b.Property<int>("IdCliente")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<int?>("ComunaClienteIdComuna")
                        .HasColumnType("int");

                    b.Property<string>("DireccionCliente")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<bool>("EsMayorista")
                        .HasColumnType("bit");

                    b.Property<string>("NombreCliente")
                        .HasMaxLength(75)
                        .HasColumnType("nvarchar(75)");

                    b.Property<int?>("RegionClienteIdRegion")
                        .HasColumnType("int");

                    b.Property<string>("RutCliente")
                        .HasMaxLength(10)
                        .HasColumnType("nvarchar(10)");

                    b.Property<int?>("SectorClienteIdSector")
                        .HasColumnType("int");

                    b.HasKey("IdCliente");

                    b.HasIndex("ComunaClienteIdComuna");

                    b.HasIndex("RegionClienteIdRegion");

                    b.HasIndex("SectorClienteIdSector");

                    b.ToTable("Clientes");
                });

            modelBuilder.Entity("FrutosElqui.Core.Cotizaciones.DetalleCotizacion", b =>
                {
                    b.Property<Guid>("IdDetalleCotizacion")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<int>("Cantidad")
                        .HasColumnType("int");

                    b.Property<int?>("ProductoIdProducto")
                        .HasColumnType("int");

                    b.Property<int?>("SucursalOrigenIdSucursal")
                        .HasColumnType("int");

                    b.HasKey("IdDetalleCotizacion");

                    b.HasIndex("ProductoIdProducto");

                    b.HasIndex("SucursalOrigenIdSucursal");

                    b.ToTable("DetallesDeCotizaciones");
                });

            modelBuilder.Entity("FrutosElqui.Core.Cotizaciones.EstadoCotizacion", b =>
                {
                    b.Property<int>("IdEstadoCotizacion")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("NombreEstadoCotizacion")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.HasKey("IdEstadoCotizacion");

                    b.ToTable("EstadoCotizaciones");
                });

            modelBuilder.Entity("FrutosElqui.Core.Misc.Banco", b =>
                {
                    b.Property<int>("IdBanco")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("NombreBanco")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.HasKey("IdBanco");

                    b.ToTable("Bancos");
                });

            modelBuilder.Entity("FrutosElqui.Core.Misc.Categoria", b =>
                {
                    b.Property<int>("IdCategoria")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("NombreCategoria")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.HasKey("IdCategoria");

                    b.ToTable("Categorias");
                });

            modelBuilder.Entity("FrutosElqui.Core.Misc.Comuna", b =>
                {
                    b.Property<int>("IdComuna")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("NombreComuna")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.HasKey("IdComuna");

                    b.ToTable("Comunas");
                });

            modelBuilder.Entity("FrutosElqui.Core.Misc.Courrier", b =>
                {
                    b.Property<int>("IdCourrier")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("NombreCourrier")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.HasKey("IdCourrier");

                    b.ToTable("Courriers");
                });

            modelBuilder.Entity("FrutosElqui.Core.Misc.Cuenta", b =>
                {
                    b.Property<Guid>("IdCuenta")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("CicloFacturacion")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("DescripcionCuenta")
                        .HasMaxLength(200)
                        .HasColumnType("nvarchar(200)");

                    b.Property<int>("DiaFacturacion")
                        .HasColumnType("int");

                    b.Property<long>("MontoDeclarado")
                        .HasColumnType("bigint");

                    b.Property<string>("NombreCuenta")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.HasKey("IdCuenta");

                    b.ToTable("Cuentas");
                });

            modelBuilder.Entity("FrutosElqui.Core.Misc.DetalleInventarios", b =>
                {
                    b.Property<int>("IdDetalleInventario")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<int>("CantidadDisponible")
                        .HasColumnType("int");

                    b.Property<int?>("ProductoIdProducto")
                        .HasColumnType("int");

                    b.Property<int?>("SucursalIdSucursal")
                        .HasColumnType("int");

                    b.HasKey("IdDetalleInventario");

                    b.HasIndex("ProductoIdProducto");

                    b.HasIndex("SucursalIdSucursal");

                    b.ToTable("Inventarios");
                });

            modelBuilder.Entity("FrutosElqui.Core.Misc.DocumentoTributario", b =>
                {
                    b.Property<int>("IdDte")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("NombreDte")
                        .HasMaxLength(75)
                        .HasColumnType("nvarchar(75)");

                    b.HasKey("IdDte");

                    b.ToTable("DocumentosTributarios");
                });

            modelBuilder.Entity("FrutosElqui.Core.Misc.EgresoDinero", b =>
                {
                    b.Property<int>("IdEgreso")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<int>("CantidadEgresada")
                        .HasColumnType("int");

                    b.Property<DateTime>("FechaEgreso")
                        .HasColumnType("datetime2");

                    b.Property<string>("Observacion")
                        .HasMaxLength(250)
                        .HasColumnType("nvarchar(250)");

                    b.Property<int?>("SucursalOrigenIdSucursal")
                        .HasColumnType("int");

                    b.HasKey("IdEgreso");

                    b.HasIndex("SucursalOrigenIdSucursal");

                    b.ToTable("EgresosDineros");
                });

            modelBuilder.Entity("FrutosElqui.Core.Misc.Giro", b =>
                {
                    b.Property<int>("IdGiro")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<long>("IdGiroSii")
                        .HasColumnType("bigint");

                    b.Property<string>("NombreGiro")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.HasKey("IdGiro");

                    b.ToTable("Giros");
                });

            modelBuilder.Entity("FrutosElqui.Core.Misc.IngresoDinero", b =>
                {
                    b.Property<int>("IdIngreso")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<int>("CantidadIngresado")
                        .HasColumnType("int");

                    b.Property<DateTime>("FechaIngreso")
                        .HasColumnType("datetime2");

                    b.Property<string>("Observacion")
                        .HasMaxLength(250)
                        .HasColumnType("nvarchar(250)");

                    b.Property<int?>("SucursalOrigenIdSucursal")
                        .HasColumnType("int");

                    b.HasKey("IdIngreso");

                    b.HasIndex("SucursalOrigenIdSucursal");

                    b.ToTable("IngresosDineros");
                });

            modelBuilder.Entity("FrutosElqui.Core.Misc.Medida", b =>
                {
                    b.Property<int>("IdMedida")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("NombreMedida")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.HasKey("IdMedida");

                    b.ToTable("Medidas");
                });

            modelBuilder.Entity("FrutosElqui.Core.Misc.Region", b =>
                {
                    b.Property<int>("IdRegion")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("NombreRegion")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.HasKey("IdRegion");

                    b.ToTable("Regiones");
                });

            modelBuilder.Entity("FrutosElqui.Core.Misc.Sabor", b =>
                {
                    b.Property<int>("IdSabor")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("NombreSabor")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.HasKey("IdSabor");

                    b.ToTable("Sabores");
                });

            modelBuilder.Entity("FrutosElqui.Core.Misc.Sector", b =>
                {
                    b.Property<int>("IdSector")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("NombreSector")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.HasKey("IdSector");

                    b.ToTable("Sectores");
                });

            modelBuilder.Entity("FrutosElqui.Core.Misc.Sucursal", b =>
                {
                    b.Property<int>("IdSucursal")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("Calle")
                        .HasMaxLength(150)
                        .HasColumnType("nvarchar(150)");

                    b.Property<int?>("ComunaIdComuna")
                        .HasColumnType("int");

                    b.Property<string>("JefeSucursal")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("NombreSucursal")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("NumeroTelefonico")
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)");

                    b.Property<int?>("RegionIdRegion")
                        .HasColumnType("int");

                    b.Property<int?>("TipoSucursalIdTipoSucursal")
                        .HasColumnType("int");

                    b.HasKey("IdSucursal");

                    b.HasIndex("ComunaIdComuna");

                    b.HasIndex("RegionIdRegion");

                    b.HasIndex("TipoSucursalIdTipoSucursal");

                    b.ToTable("Sucursales");
                });

            modelBuilder.Entity("FrutosElqui.Core.Misc.TipoCuenta", b =>
                {
                    b.Property<int>("IdTipoCuenta")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("NombreTipoCuenta")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.HasKey("IdTipoCuenta");

                    b.ToTable("TipoCuentas");
                });

            modelBuilder.Entity("FrutosElqui.Core.Misc.TipoPago", b =>
                {
                    b.Property<int>("IdTipoPago")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("NombreTipoPago")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.HasKey("IdTipoPago");

                    b.ToTable("TipoPagos");
                });

            modelBuilder.Entity("FrutosElqui.Core.Misc.TipoSucursal", b =>
                {
                    b.Property<int>("IdTipoSucursal")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("NombreTipoSucursal")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.HasKey("IdTipoSucursal");

                    b.ToTable("TipoSucursales");
                });

            modelBuilder.Entity("FrutosElqui.Core.Ofertas.DetalleOferta", b =>
                {
                    b.Property<Guid>("GuidDetalle")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<int>("CantidadProducto")
                        .HasColumnType("int");

                    b.Property<Guid?>("OfertaGuidOferta")
                        .HasColumnType("uniqueidentifier");

                    b.Property<int?>("ProductoIdProducto")
                        .HasColumnType("int");

                    b.HasKey("GuidDetalle");

                    b.HasIndex("OfertaGuidOferta");

                    b.HasIndex("ProductoIdProducto");

                    b.ToTable("DetallesDeOfertas");
                });

            modelBuilder.Entity("FrutosElqui.Core.Ofertas.Oferta", b =>
                {
                    b.Property<Guid>("GuidOferta")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime>("FechaCreacion")
                        .HasColumnType("datetime2");

                    b.Property<string>("NombreOferta")
                        .HasMaxLength(200)
                        .HasColumnType("nvarchar(200)");

                    b.Property<int>("PrecioOferta")
                        .HasColumnType("int");

                    b.HasKey("GuidOferta");

                    b.ToTable("Ofertas");
                });

            modelBuilder.Entity("FrutosElqui.Core.Productos.Producto", b =>
                {
                    b.Property<int>("IdProducto")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<int?>("CategoriaProductoIdCategoria")
                        .HasColumnType("int");

                    b.Property<int>("Costo")
                        .HasColumnType("int");

                    b.Property<string>("DescripcionProducto")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<DateTime>("FechaCreacionProducto")
                        .HasColumnType("datetime2");

                    b.Property<int?>("MedidaProductoIdMedida")
                        .HasColumnType("int");

                    b.Property<string>("NombreBusqueda")
                        .IsRequired()
                        .HasMaxLength(150)
                        .HasColumnType("nvarchar(150)");

                    b.Property<string>("NombreProducto")
                        .HasMaxLength(75)
                        .HasColumnType("nvarchar(75)");

                    b.Property<int?>("PrecioMayorista")
                        .HasColumnType("int");

                    b.Property<int>("PrecioTotal")
                        .HasColumnType("int");

                    b.Property<int?>("ProveedorProductoIdProveedor")
                        .HasColumnType("int");

                    b.Property<int?>("SaborProductoIdSabor")
                        .HasColumnType("int");

                    b.HasKey("IdProducto");

                    b.HasIndex("CategoriaProductoIdCategoria");

                    b.HasIndex("MedidaProductoIdMedida");

                    b.HasIndex("ProveedorProductoIdProveedor");

                    b.HasIndex("SaborProductoIdSabor");

                    b.ToTable("Productos");
                });

            modelBuilder.Entity("FrutosElqui.Core.Proveedores.Proveedor", b =>
                {
                    b.Property<int>("IdProveedor")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<int?>("BancoProveedorIdBanco")
                        .HasColumnType("int");

                    b.Property<int?>("DiaFacturacion")
                        .HasColumnType("int");

                    b.Property<string>("DireccionProveedor")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<int?>("GiroProveedorIdGiro")
                        .HasColumnType("int");

                    b.Property<string>("NombreProveedor")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<int>("NumeroCuenta")
                        .HasColumnType("int");

                    b.Property<string>("RutProveedor")
                        .HasMaxLength(12)
                        .HasColumnType("nvarchar(12)");

                    b.Property<int?>("TipoCuentaProveedorIdTipoCuenta")
                        .HasColumnType("int");

                    b.Property<int?>("TipoPagoProveedorIdTipoPago")
                        .HasColumnType("int");

                    b.HasKey("IdProveedor");

                    b.HasIndex("BancoProveedorIdBanco");

                    b.HasIndex("GiroProveedorIdGiro");

                    b.HasIndex("TipoCuentaProveedorIdTipoCuenta");

                    b.HasIndex("TipoPagoProveedorIdTipoPago");

                    b.ToTable("Proveedores");
                });

            modelBuilder.Entity("FrutosElqui.Core.Ventas.DetalleVenta", b =>
                {
                    b.Property<Guid>("IdDetalle")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<int>("CantidadProducto")
                        .HasColumnType("int");

                    b.Property<int>("IdProducto")
                        .HasColumnType("int");

                    b.Property<int>("IdProveedor")
                        .HasColumnType("int");

                    b.Property<string>("NombreProducto")
                        .HasMaxLength(150)
                        .HasColumnType("nvarchar(150)");

                    b.Property<string>("NombreProveedor")
                        .HasMaxLength(150)
                        .HasColumnType("nvarchar(150)");

                    b.Property<int>("TotalProducto")
                        .HasColumnType("int");

                    b.Property<Guid?>("VentaIdVenta")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("IdDetalle");

                    b.HasIndex("VentaIdVenta");

                    b.ToTable("DetallesVentas");
                });

            modelBuilder.Entity("FrutosElqui.Core.Ventas.OfertasEnVenta", b =>
                {
                    b.Property<Guid>("IdOfertasVenta")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<int>("CantidadOferta")
                        .HasColumnType("int");

                    b.Property<Guid>("GuidOferta")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("NombreOferta")
                        .HasMaxLength(150)
                        .HasColumnType("nvarchar(150)");

                    b.Property<Guid?>("VentaIdVenta")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("IdOfertasVenta");

                    b.HasIndex("VentaIdVenta");

                    b.ToTable("OfertasVentas");
                });

            modelBuilder.Entity("FrutosElqui.Core.Ventas.Venta", b =>
                {
                    b.Property<Guid>("IdVenta")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<int>("Descuento")
                        .HasColumnType("int");

                    b.Property<DateTime>("FechaVenta")
                        .HasColumnType("datetime2");

                    b.Property<string>("NombreUsuarioVenta")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("RutUsuarioVenta")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int?>("SucursalIdSucursal")
                        .HasColumnType("int");

                    b.Property<int?>("TipoPagoIdTipoPago")
                        .HasColumnType("int");

                    b.Property<int>("Total")
                        .HasColumnType("int");

                    b.HasKey("IdVenta");

                    b.HasIndex("SucursalIdSucursal");

                    b.HasIndex("TipoPagoIdTipoPago");

                    b.ToTable("Ventas");
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityRole", b =>
                {
                    b.Property<string>("Id")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("ConcurrencyStamp")
                        .IsConcurrencyToken()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Name")
                        .HasMaxLength(256)
                        .HasColumnType("nvarchar(256)");

                    b.Property<string>("NormalizedName")
                        .HasMaxLength(256)
                        .HasColumnType("nvarchar(256)");

                    b.HasKey("Id");

                    b.HasIndex("NormalizedName")
                        .IsUnique()
                        .HasDatabaseName("RoleNameIndex")
                        .HasFilter("[NormalizedName] IS NOT NULL");

                    b.ToTable("AspNetRoles");
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityRoleClaim<string>", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("ClaimType")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("ClaimValue")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("RoleId")
                        .IsRequired()
                        .HasColumnType("nvarchar(450)");

                    b.HasKey("Id");

                    b.HasIndex("RoleId");

                    b.ToTable("AspNetRoleClaims");
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUser", b =>
                {
                    b.Property<string>("Id")
                        .HasColumnType("nvarchar(450)");

                    b.Property<int>("AccessFailedCount")
                        .HasColumnType("int");

                    b.Property<string>("ConcurrencyStamp")
                        .IsConcurrencyToken()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Discriminator")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Email")
                        .HasMaxLength(256)
                        .HasColumnType("nvarchar(256)");

                    b.Property<bool>("EmailConfirmed")
                        .HasColumnType("bit");

                    b.Property<bool>("LockoutEnabled")
                        .HasColumnType("bit");

                    b.Property<DateTimeOffset?>("LockoutEnd")
                        .HasColumnType("datetimeoffset");

                    b.Property<string>("NormalizedEmail")
                        .HasMaxLength(256)
                        .HasColumnType("nvarchar(256)");

                    b.Property<string>("NormalizedUserName")
                        .HasMaxLength(256)
                        .HasColumnType("nvarchar(256)");

                    b.Property<string>("PasswordHash")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("PhoneNumber")
                        .HasColumnType("nvarchar(max)");

                    b.Property<bool>("PhoneNumberConfirmed")
                        .HasColumnType("bit");

                    b.Property<string>("SecurityStamp")
                        .HasColumnType("nvarchar(max)");

                    b.Property<bool>("TwoFactorEnabled")
                        .HasColumnType("bit");

                    b.Property<string>("UserName")
                        .HasMaxLength(256)
                        .HasColumnType("nvarchar(256)");

                    b.HasKey("Id");

                    b.HasIndex("NormalizedEmail")
                        .HasDatabaseName("EmailIndex");

                    b.HasIndex("NormalizedUserName")
                        .IsUnique()
                        .HasDatabaseName("UserNameIndex")
                        .HasFilter("[NormalizedUserName] IS NOT NULL");

                    b.ToTable("AspNetUsers");

                    b.HasDiscriminator<string>("Discriminator").HasValue("IdentityUser");
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserClaim<string>", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("ClaimType")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("ClaimValue")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("UserId")
                        .IsRequired()
                        .HasColumnType("nvarchar(450)");

                    b.HasKey("Id");

                    b.HasIndex("UserId");

                    b.ToTable("AspNetUserClaims");
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserLogin<string>", b =>
                {
                    b.Property<string>("LoginProvider")
                        .HasMaxLength(128)
                        .HasColumnType("nvarchar(128)");

                    b.Property<string>("ProviderKey")
                        .HasMaxLength(128)
                        .HasColumnType("nvarchar(128)");

                    b.Property<string>("ProviderDisplayName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("UserId")
                        .IsRequired()
                        .HasColumnType("nvarchar(450)");

                    b.HasKey("LoginProvider", "ProviderKey");

                    b.HasIndex("UserId");

                    b.ToTable("AspNetUserLogins");
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserRole<string>", b =>
                {
                    b.Property<string>("UserId")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("RoleId")
                        .HasColumnType("nvarchar(450)");

                    b.HasKey("UserId", "RoleId");

                    b.HasIndex("RoleId");

                    b.ToTable("AspNetUserRoles");
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserToken<string>", b =>
                {
                    b.Property<string>("UserId")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("LoginProvider")
                        .HasMaxLength(128)
                        .HasColumnType("nvarchar(128)");

                    b.Property<string>("Name")
                        .HasMaxLength(128)
                        .HasColumnType("nvarchar(128)");

                    b.Property<string>("Value")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("UserId", "LoginProvider", "Name");

                    b.ToTable("AspNetUserTokens");
                });

            modelBuilder.Entity("FrutosElqui.Core.Usuarios.AppUser", b =>
                {
                    b.HasBaseType("Microsoft.AspNetCore.Identity.IdentityUser");

                    b.Property<int?>("ComunaUsuarioIdComuna")
                        .HasColumnType("int");

                    b.Property<string>("Direccion")
                        .HasMaxLength(150)
                        .HasColumnType("nvarchar(150)");

                    b.Property<string>("Nombre")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<int?>("RegionUsuarioIdRegion")
                        .HasColumnType("int");

                    b.Property<string>("Role")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Rut")
                        .IsRequired()
                        .HasMaxLength(15)
                        .HasColumnType("nvarchar(15)");

                    b.HasIndex("ComunaUsuarioIdComuna");

                    b.HasIndex("RegionUsuarioIdRegion");

                    b.HasDiscriminator().HasValue("AppUser");
                });

            modelBuilder.Entity("FrutosElqui.Core.Clientes.Cliente", b =>
                {
                    b.HasOne("FrutosElqui.Core.Misc.Comuna", "ComunaCliente")
                        .WithMany()
                        .HasForeignKey("ComunaClienteIdComuna");

                    b.HasOne("FrutosElqui.Core.Misc.Region", "RegionCliente")
                        .WithMany()
                        .HasForeignKey("RegionClienteIdRegion");

                    b.HasOne("FrutosElqui.Core.Misc.Sector", "SectorCliente")
                        .WithMany()
                        .HasForeignKey("SectorClienteIdSector");

                    b.Navigation("ComunaCliente");

                    b.Navigation("RegionCliente");

                    b.Navigation("SectorCliente");
                });

            modelBuilder.Entity("FrutosElqui.Core.Cotizaciones.DetalleCotizacion", b =>
                {
                    b.HasOne("FrutosElqui.Core.Productos.Producto", "Producto")
                        .WithMany()
                        .HasForeignKey("ProductoIdProducto");

                    b.HasOne("FrutosElqui.Core.Misc.Sucursal", "SucursalOrigen")
                        .WithMany()
                        .HasForeignKey("SucursalOrigenIdSucursal");

                    b.Navigation("Producto");

                    b.Navigation("SucursalOrigen");
                });

            modelBuilder.Entity("FrutosElqui.Core.Misc.DetalleInventarios", b =>
                {
                    b.HasOne("FrutosElqui.Core.Productos.Producto", "Producto")
                        .WithMany()
                        .HasForeignKey("ProductoIdProducto");

                    b.HasOne("FrutosElqui.Core.Misc.Sucursal", "Sucursal")
                        .WithMany()
                        .HasForeignKey("SucursalIdSucursal");

                    b.Navigation("Producto");

                    b.Navigation("Sucursal");
                });

            modelBuilder.Entity("FrutosElqui.Core.Misc.EgresoDinero", b =>
                {
                    b.HasOne("FrutosElqui.Core.Misc.Sucursal", "SucursalOrigen")
                        .WithMany()
                        .HasForeignKey("SucursalOrigenIdSucursal");

                    b.Navigation("SucursalOrigen");
                });

            modelBuilder.Entity("FrutosElqui.Core.Misc.IngresoDinero", b =>
                {
                    b.HasOne("FrutosElqui.Core.Misc.Sucursal", "SucursalOrigen")
                        .WithMany()
                        .HasForeignKey("SucursalOrigenIdSucursal");

                    b.Navigation("SucursalOrigen");
                });

            modelBuilder.Entity("FrutosElqui.Core.Misc.Sucursal", b =>
                {
                    b.HasOne("FrutosElqui.Core.Misc.Comuna", "Comuna")
                        .WithMany()
                        .HasForeignKey("ComunaIdComuna");

                    b.HasOne("FrutosElqui.Core.Misc.Region", "Region")
                        .WithMany()
                        .HasForeignKey("RegionIdRegion");

                    b.HasOne("FrutosElqui.Core.Misc.TipoSucursal", "TipoSucursal")
                        .WithMany()
                        .HasForeignKey("TipoSucursalIdTipoSucursal");

                    b.Navigation("Comuna");

                    b.Navigation("Region");

                    b.Navigation("TipoSucursal");
                });

            modelBuilder.Entity("FrutosElqui.Core.Ofertas.DetalleOferta", b =>
                {
                    b.HasOne("FrutosElqui.Core.Ofertas.Oferta", null)
                        .WithMany("ProductosEnOferta")
                        .HasForeignKey("OfertaGuidOferta");

                    b.HasOne("FrutosElqui.Core.Productos.Producto", "Producto")
                        .WithMany()
                        .HasForeignKey("ProductoIdProducto");

                    b.Navigation("Producto");
                });

            modelBuilder.Entity("FrutosElqui.Core.Productos.Producto", b =>
                {
                    b.HasOne("FrutosElqui.Core.Misc.Categoria", "CategoriaProducto")
                        .WithMany()
                        .HasForeignKey("CategoriaProductoIdCategoria");

                    b.HasOne("FrutosElqui.Core.Misc.Medida", "MedidaProducto")
                        .WithMany()
                        .HasForeignKey("MedidaProductoIdMedida");

                    b.HasOne("FrutosElqui.Core.Proveedores.Proveedor", "ProveedorProducto")
                        .WithMany()
                        .HasForeignKey("ProveedorProductoIdProveedor");

                    b.HasOne("FrutosElqui.Core.Misc.Sabor", "SaborProducto")
                        .WithMany()
                        .HasForeignKey("SaborProductoIdSabor");

                    b.Navigation("CategoriaProducto");

                    b.Navigation("MedidaProducto");

                    b.Navigation("ProveedorProducto");

                    b.Navigation("SaborProducto");
                });

            modelBuilder.Entity("FrutosElqui.Core.Proveedores.Proveedor", b =>
                {
                    b.HasOne("FrutosElqui.Core.Misc.Banco", "BancoProveedor")
                        .WithMany()
                        .HasForeignKey("BancoProveedorIdBanco");

                    b.HasOne("FrutosElqui.Core.Misc.Giro", "GiroProveedor")
                        .WithMany()
                        .HasForeignKey("GiroProveedorIdGiro");

                    b.HasOne("FrutosElqui.Core.Misc.TipoCuenta", "TipoCuentaProveedor")
                        .WithMany()
                        .HasForeignKey("TipoCuentaProveedorIdTipoCuenta");

                    b.HasOne("FrutosElqui.Core.Misc.TipoPago", "TipoPagoProveedor")
                        .WithMany()
                        .HasForeignKey("TipoPagoProveedorIdTipoPago");

                    b.Navigation("BancoProveedor");

                    b.Navigation("GiroProveedor");

                    b.Navigation("TipoCuentaProveedor");

                    b.Navigation("TipoPagoProveedor");
                });

            modelBuilder.Entity("FrutosElqui.Core.Ventas.DetalleVenta", b =>
                {
                    b.HasOne("FrutosElqui.Core.Ventas.Venta", null)
                        .WithMany("DetallesVenta")
                        .HasForeignKey("VentaIdVenta");
                });

            modelBuilder.Entity("FrutosElqui.Core.Ventas.OfertasEnVenta", b =>
                {
                    b.HasOne("FrutosElqui.Core.Ventas.Venta", null)
                        .WithMany("OfertasEnVenta")
                        .HasForeignKey("VentaIdVenta");
                });

            modelBuilder.Entity("FrutosElqui.Core.Ventas.Venta", b =>
                {
                    b.HasOne("FrutosElqui.Core.Misc.Sucursal", "Sucursal")
                        .WithMany()
                        .HasForeignKey("SucursalIdSucursal");

                    b.HasOne("FrutosElqui.Core.Misc.TipoPago", "TipoPago")
                        .WithMany()
                        .HasForeignKey("TipoPagoIdTipoPago");

                    b.Navigation("Sucursal");

                    b.Navigation("TipoPago");
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityRoleClaim<string>", b =>
                {
                    b.HasOne("Microsoft.AspNetCore.Identity.IdentityRole", null)
                        .WithMany()
                        .HasForeignKey("RoleId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserClaim<string>", b =>
                {
                    b.HasOne("Microsoft.AspNetCore.Identity.IdentityUser", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserLogin<string>", b =>
                {
                    b.HasOne("Microsoft.AspNetCore.Identity.IdentityUser", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserRole<string>", b =>
                {
                    b.HasOne("Microsoft.AspNetCore.Identity.IdentityRole", null)
                        .WithMany()
                        .HasForeignKey("RoleId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Microsoft.AspNetCore.Identity.IdentityUser", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserToken<string>", b =>
                {
                    b.HasOne("Microsoft.AspNetCore.Identity.IdentityUser", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("FrutosElqui.Core.Usuarios.AppUser", b =>
                {
                    b.HasOne("FrutosElqui.Core.Misc.Comuna", "ComunaUsuario")
                        .WithMany()
                        .HasForeignKey("ComunaUsuarioIdComuna");

                    b.HasOne("FrutosElqui.Core.Misc.Region", "RegionUsuario")
                        .WithMany()
                        .HasForeignKey("RegionUsuarioIdRegion");

                    b.Navigation("ComunaUsuario");

                    b.Navigation("RegionUsuario");
                });

            modelBuilder.Entity("FrutosElqui.Core.Ofertas.Oferta", b =>
                {
                    b.Navigation("ProductosEnOferta");
                });

            modelBuilder.Entity("FrutosElqui.Core.Ventas.Venta", b =>
                {
                    b.Navigation("DetallesVenta");

                    b.Navigation("OfertasEnVenta");
                });
#pragma warning restore 612, 618
        }
    }
}
